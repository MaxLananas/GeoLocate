...
public final class GeoPath {
    ...
    public GeoPoint getBoundingBoxCenter() {
        if (points.isEmpty()) return null;
        double minLat = points.stream().mapToDouble(GeoPoint::latitude).min().orElse(0);
        double maxLat = points.stream().mapToDouble(GeoPoint::latitude).max().orElse(0);
        double minLon = points.stream().mapToDouble(GeoPoint::longitude).min().orElse(0);
        double maxLon = points.stream().mapToDouble(GeoPoint::longitude).max().orElse(0);
