...
public enum MapProjection {
    ...
    // Shared constants — avoid repeated division at runtime
    protected static final double HALF_PI    = Math.PI / 2.0;
    protected static final double QUARTER_PI = Math.PI / 4.0;
    ...
    /** Mercator y-value for a latitude in radians. */
    protected static double mercY(double latRad) {
        return Math.log(Math.tan(QUARTER_PI + latRad * 0.5));
    }

    protected static double clampLat(double lat) {
        return Math.max(-90.0, Math.min(90.0, lat));
    }

    protected static double clampLon(double lon) {
        double r = lon % 360.0;
        if (r >  180.0) r -= 360.0;
        if (r < -180.0) r += 360.0;
        return r;
